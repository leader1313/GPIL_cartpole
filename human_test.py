...
# 
# python3 human_test.py CartPole-v1
#
...
def key_press(key, mod):
    ...
    if key==32: 
        ...
        human_action_start = not human_action_start
    a = int( key - ord('0') )
    ...
    for i in range(max_episode):
        ...
        while 1:
            ...
            while human_sets_pause:
                ...
                time.sleep(0.1)
            time.sleep(0.1)
        print("\t ["+ str(i+1) +"] times : timesteps %i reward %0.2f" % (total_timesteps, total_reward))
        # reward += total_reward
        # reward = reward/sim_iter
        # print("[Finish!!] Episode %i mean of reward : %0.2f" % (i+1, reward))
        graph['reward'].append(total_reward)
    X = graph['episode_num']
    Y = graph['reward']
    plt.xlabel("episode_num")
    plt.ylabel("reward")
    plt.title("%s learning rate" % (learner))
    plt.plot(X, Y, "*")
    line = plt.plot(X, Y)
    plt.ylim(0.0, 600)
    breakPoint = True
